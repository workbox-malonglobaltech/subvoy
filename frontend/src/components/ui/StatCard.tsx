...
/** Executive KPI card — icon chip + corner trend badge + label + large value. */
export function StatCard({ label, value, trend, trendUpIsGood, icon, hint, className }: StatCardProps) {
  const up = (trend ?? 0) > 0;
    ...
    <div className={cn('rounded-2xl border border-line bg-surface p-5 shadow-card', className)}>
      {(icon || trend != null) && (
        <div className="mb-3 flex items-center justify-between">
          {icon ? (
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary-50 text-primary-600">{icon}</span>
          ) : <span />}
          {trend != null && (
            <span className={cn(
