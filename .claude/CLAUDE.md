...
- [ ] **4 pre-existing test failures (block CI until triaged)** — present since the initial commit, unrelated to autopay:
  - `auth.test.ts › GET /auth/me › returns 404 when the user is no longer in the DB` — gets 401, not 404 (the `authenticate` middleware rejects unknown users before the route runs). Decide the intended contract (401 is the more secure answer) and update the test or the route.
  - `auth.test.ts › GET /auth/me › returns 500 when the database throws` and `› POST /auth/logout › still returns 200 when token-revocation DB update fails` — same middleware-ordering drift.
  - `imports.test.ts › POST /imports/csv › returns 400 for a non-CSV file type`.
  CI runs the full suite (no masking), so it will be red until these are fixed.
